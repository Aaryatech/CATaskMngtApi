			...
			for (int j = 0; j < empTypeIds.size(); j++) {
				...
				try {
					...
					try {
						...
						String subject = "Test Email-(New) Please Ignore-";// " Login Credentials For RUSA Login ";
						...
						if (empTypeIds.get(j) == 5) {
							email.append("Work Log Data Test Sample Emp " + crDtTime);
						} else if (empTypeIds.get(j) == 3) {
							email.append("Work Log Data Test Sample Manager " + crDtTime);
						} else if (empTypeIds.get(j) == 4) {
							email.append("Work Log Data Test TL " + crDtTime);
						}
