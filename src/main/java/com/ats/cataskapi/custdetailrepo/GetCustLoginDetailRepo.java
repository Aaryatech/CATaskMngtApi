...
public interface GetCustLoginDetailRepo extends JpaRepository<GetCustLoginDetail, Integer> {
	 ...
	 * +
	 * " ELSE COALESCE(( SELECT m_cust_group.cust_group_name FROM m_cust_group,m_cust_header WHERE m_cust_group.cust_group_id=m_cust_header.cust_group_id "
	 * +
	 ...
	 * +
	 * " m_cust_detail.login_remark,m_cust_detail.cust_detail_id,m_cust_detail.cust_id "
	...
	@Query(value=" SELECT   m_cust_header.cust_firm_name, " + 
	...
	List<GetCustLoginDetail> getGetCustLoginDetailByCustId(@Param("custId") int custId);
