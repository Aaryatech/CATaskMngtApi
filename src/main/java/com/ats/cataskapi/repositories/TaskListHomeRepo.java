...
public interface TaskListHomeRepo extends JpaRepository<TaskListHome, Integer> {
	...
	@Query(value="SELECT"
			...
			"			  ELSE COALESCE(( SELECT m_cust_group.cust_group_name \n" + 
			"                             FROM m_cust_group\n" + 
			"                             WHERE m_cust_group.cust_group_id=m_cust_header.cust_group_id	 \n" + 
			"	  						AND m_cust_group.del_status=1 ),0) END AS cust_group_name,\n" + 
...
/*****************************************************************************************/
	...
	@Query(value="SELECT\n" + 
			...
			"                                 WHERE m_cust_group.cust_group_id=m_cust_header.cust_group_id \n" + 
			"	 \n" + 
			"	  AND m_cust_group.del_status=1 ),0) \n" + 
