...
public interface EmpListForDashboardRepo extends JpaRepository<EmpListForDashboard, Integer>{
	...
	@Query(value="select emp_id, emp_name from m_emp where emp_id in (:ids) and del_status=1",nativeQuery=true)
	List<EmpListForDashboard> getempList(@Param("ids") ArrayList<String> ids);
