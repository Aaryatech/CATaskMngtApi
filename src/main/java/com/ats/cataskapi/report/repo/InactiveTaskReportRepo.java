...
public interface InactiveTaskReportRepo extends JpaRepository<InactiveTaskReport, Integer>{
	...
	@Query(value="SELECT\n" + 
			...
			"            t_daily_work_log\n" + 
			"        WHERE\n" + 
			"            t_tasks.task_status = :status AND t_tasks.del_status = 1 AND t_tasks.is_active = 1 AND t_daily_work_log.task_id = t_tasks.task_id AND t_tasks.task_completion_date BETWEEN :fromDate1 AND :toDate1 \n" + 
			"        GROUP BY\n" + 
